...
const isolatedWorkspace = fs.mkdtempSync(path.join(os.tmpdir(), 'cache-test-workspace-'));
fs.writeFileSync(path.join(isolatedWorkspace, '.cache-the-planet.json'), '{}');
process.env.GITHUB_WORKSPACE = isolatedWorkspace;
process.on('exit', () => fs.rmSync(isolatedWorkspace, { recursive: true, force: true }));
